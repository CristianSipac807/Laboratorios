...
void llamaCiclo()
{
    ...
    char opcion;  //Variable que controla el ciclo de repeticion
    bool repetir = true;  //Variable booleana del ciclo de repeticion
    //Declaracion de variables flotantes para el calculo de promedios
    float promedio_facultad_1;
    float promedio_facultad_2;
    float promedio_facultad_3;
    //Declaraci�n de matriz que almacena los nombres de los alumnos
...
{
    //Calculo de valores aleatorios tomando como base la nota minima y maxima del parcial
    return minimo + rand() / (RAND_MAX / (maximo - minimo + 1) + 1);
}

void llenarMatriz(float matriz[NUMERO_ALUMNOS][NUMERO_NOTAS + 1])
{
    //Funcion que permite llenar cada matriz con valores aleatorios
    int y, x;
...
{
    //Funcion que despliega titulo de cada nota calculada
    int x;
